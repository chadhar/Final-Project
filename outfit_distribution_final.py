...
def generate_prob_table(n_items, n_persons, n_replications):
    ...
    Uses a pseudo-random sample of person measures from a normally-distributed population with mean = 0 logits
...
def generate_summary_stats(std_outfit,n_items,n_persons,n_replications):
    ...
    for i in range(np.size(std_outfit, 1)):
        prank_pos_2 = np.append(prank_pos_2, stats.percentileofscore(std_outfit[:, i], 2, kind='weak'))
        prank_neg_2 = np.append(prank_neg_2, stats.percentileofscore(std_outfit[:, i], -2, kind='weak'))
...
def generate_plots(std_outfit, n_items, n_persons):
    ...
    for i in range(np.size(std_outfit, 1)):
        ...
        plt.show()
    print("Standardized outfit statistics plots saved.")
...
if __name__ == '__main__':
    ...
    # Generate probability of correct response table.
    prob_of_corr_response = generate_prob_table(n_items,n_persons, n_replications)
    std_outfit_stats = np.array([])

    # Calculate standardized outfit statistics over the number of replications.
    for replication in range(n_replications):
        # Generate a random sample of probabilities
        prob_sample = replication_sample(n_persons, n_items)

        # Generate a sample response matrix
        sample_resp_matrix = generate_response_matrix(prob_of_corr_response,prob_sample)

        # Check if any item has all 1s or 1s. If yes, then generate another sample response matrix
        while check_extreme_scores(sample_resp_matrix) == 1:
            prob_sample = replication_sample(n_persons, n_items)
            sample_resp_matrix = generate_response_matrix(prob_of_corr_response, prob_sample)
            # If the sample response matrix does not have any item with all 0s or 1s, break out of the file loop.
            if check_extreme_scores(sample_resp_matrix) == 0:
                break
        # For first replication
        if std_outfit_stats.size == 0:
            std_outfit_stats = calculate_outfit(prob_of_corr_response,sample_resp_matrix)
        # For replications after the first one
        else:
    ...
    # Generate plots of distribution of outfit statistics
